...
bool CopyNode::CheckCooperativeTensorCopy(Target target) const {
  if (TargetIsMetal(target) && IsCooperativeTensorBuffer(src)) {
...
CopyInst CopyNode::GetCopyInst(Target target, const LayoutMap &layout_map,
  ...
  } else if (CheckCooperativeTensorCopy(target)) {
    return CopyInst::kMetalCooperativeTensor;
...
Stmt CopyNode::Lower(const LowerArgs &T, arith::Analyzer *analyzer) const {
  ...
  } else if (copy_inst == CopyInst::kMetalCooperativeTensor) {
    return LowerCooperativeTensorCopy(T, analyzer);
...
Stmt CopyNode::LowerCooperativeTensorCopy(const LowerArgs &T,
                                          arith::Analyzer *analyzer) const {
  ICHECK(IsCooperativeTensorBuffer(src));
  int total_elements = 1;
  for (auto s : src->shape) {
    auto imm = s.as<IntImmNode>();
  ...
  int kTileSize = 16;
  int tile_elems = kTileSize * kTileSize;
