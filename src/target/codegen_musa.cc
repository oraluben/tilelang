...
std::string GetFP6Type(DataType type) {
  ...
  if (type.code() == DataType::kFloat6_e2m3fn) {
    ...
    LOG(FATAL) << "Unsupported FP6 type in MUSA codegen";
  }
  stream << vec << suffix;
  return stream.str();
}
...
std::string GetFP4Type(DataType type) {
  ...
  if (type.code() == DataType::kFloat4_e2m1fn) {
    ...
    LOG(FATAL) << "Unsupported FP4 type in MUSA codegen";
  }
  stream << vec << suffix;
  return stream.str();
}

CodeGenTileLangMUSA::CodeGenTileLangMUSA() {
...
void CodeGenTileLangMUSA::PrintFuncPrefix(std::ostream &os) {
...
void CodeGenTileLangMUSA::PrintExtraAttrs(const PrimFunc &f) {
...
void CodeGenTileLangMUSA::VisitStmt_(const tir::ForNode *op) {
...
void CodeGenTileLangMUSA::BindThreadIndex(const IterVar &iv) {
  ICHECK(!var_idmap_.count(iv->var.get()));
  var_idmap_[iv->var.get()] =
      CastFromTo(iv->thread_tag, DataType::UInt(32), iv->var.dtype());
}

void CodeGenTileLangMUSA::PrintType(DataType t, std::ostream &os) { // NOLINT(*)
  int lanes = t.lanes();
  if (t.is_handle()) {
...
void CodeGenTileLangMUSA::PrintVecElemLoad(const std::string &vec, DataType t,
                                           int i,
                                           std::ostream &os) { // NOLINT(*)
  if (t.is_scalar()) {
    os << vec;
  ...
  if (t.bits() == 8 && (t.is_int() || t.is_uint())) {
    ...
    if (t.lanes() <= 8) {
      os << "((mt_bfloat162*)(&(" << vec << "." << access[i / 2] << ")))->"
         << access[i % 2];
    } else {
      os << "(((mt_bfloat162*)(&(" << vec << "." << access[i / 4] << "))) + "
         << (i / 2 % 2) << ")->" << access[i % 2];
    }
  } else if (t.is_float8()) {
    os << vec;
...
void CodeGenTileLangMUSA::PrintVecElemStore(const std::string &vec, DataType t,
  ...
  if (t.bits() == 8 && (t.is_int() || t.is_uint())) {
    ...
    if (t.lanes() <= 8) {
      stream << "((mt_bfloat162*)(&(" << vec << "." << access[i / 2] << ")))->"
             << access[i % 2] << " = " << value << ";\n";
    } else {
      stream << "(((mt_bfloat162*)(&(" << vec << "." << access[i / 4]
             << "))) + " << (i / 2 % 2) << ")->" << access[i % 2] << " = "
             << value << ";\n";
    }
  } else if (t.is_float8()) {
...
void CodeGenTileLangMUSA::PrintStorageSync(const CallNode *op) {
  auto args = op->args;
...
void CodeGenTileLangMUSA::PrintStorageScope(const std::string &scope,
                                            std::ostream &os) { // NOLINT(*)
  ICHECK_NE(scope, "global")
      << "Cannot allocate global memory when targeting MUSA. You must pass "
...
std::string CodeGenTileLangMUSA::CastFromTo(std::string value, DataType from,
                                            DataType target) {
  if (from == target)
    return value;
...
void CodeGenTileLangMUSA::VisitExpr_(const CastNode *op, std::ostream &os) {
  ...
  if (from_ty.is_bfloat16() && target_ty.is_float()) {
    ...
    if (from_ty.lanes() == 2 && target_ty.lanes() == 2) {
      ...
      PrintIndent();
      stream << sret
             << " = __bfloat1622float2(*reinterpret_cast<__mt_bfloat162*>(&("
             << src << ")));\n";
      os << sret;
      return;
    } else if (from_ty.lanes() == 4 && target_ty.lanes() == 4) {
    ...
    if (from_ty.lanes() == 2 && target_ty.lanes() == 2) {
      // float2 -> bfloat162
      PrintIndent();
      stream << "*reinterpret_cast<__mt_bfloat162*>(&(" << sret
      ...
      // float4 -> bfloat162x2
      PrintIndent();
      stream << "(reinterpret_cast<__mt_bfloat162*>(&" << sret << "))[0] = "
             << "__float22bfloat162_rn(*(float2*)(&(" << src << ")));\n";
      PrintIndent();
      stream << "(reinterpret_cast<__mt_bfloat162*>(&" << sret << "))[1] = "
      ...
      stream << "(reinterpret_cast<__mt_bfloat162*>(&" << sret << "))[1] = "
             << "__float22bfloat162_rn(*((float2*)(&(" << src << "))+1));\n";
      PrintIndent();
      stream << "(reinterpret_cast<__mt_bfloat162*>(&" << sret << "))[2] = "
             << "__float22bfloat162_rn(*((float2*)(&(" << src << "))+2));\n";
      PrintIndent();
      stream << "(reinterpret_cast<__mt_bfloat162*>(&" << sret << "))[3] = "
...
void CodeGenTileLangMUSA::VisitExpr_(const MinNode *op, std::ostream &os) {
...
void CodeGenTileLangMUSA::VisitExpr_(const MaxNode *op, std::ostream &os) {
  // TODO(wt): Consider vectorized reduction and impl for other dtypes
  DataType t = op->dtype;
...
void CodeGenTileLangMUSA::PrintCallExtern(Type ret_type, String global_symbol,
  ...
  if (ret_dtype.is_fixed_length_vector()) {
    if (global_symbol == "tl::vec_max_f2" ||
        ...
        global_symbol == "tl::vec_exp2_f2" ||
        global_symbol == "tl::vec_exp2_f4" ||
...
std::string CodeGenTileLangMUSA::GetBufferRef(DataType t,
...
std::string CodeGenTileLangMUSA::GetVecLoad(DataType t,
                                            const BufferNode *buffer,
                                            PrimExpr base) {
  const VarNode *buffer_var = buffer->data.get();
  std::string scope;
...
void CodeGenTileLangMUSA::PrintVecStore(const BufferNode *buffer, DataType t,
                                        PrimExpr base,
...
/**
 * @brief Emit MUSA/TensorLib-specific code for a call expression.
 *
 * This visitor handles CallNode intrinsics and builtins that require emitting
 * MUSA/TL-specific code (inline PTX/ASM sequences, TensorLanguage runtime
 ...
 */
void CodeGenTileLangMUSA::VisitExpr_(const CallNode *op, std::ostream &os) {
...
void CodeGenTileLangMUSA::VisitStmt_(const AttrStmtNode *op) {
  if (op->attr_key == tir::attr::fragment_shape) {
    ...
    ICHECK(queue_id && queue_id->value == 0)
        << "For MUSA, the index of an async queue must be 0.";
...
void CodeGenTileLangMUSA::VisitStmt_(const AllocateNode *op) {
...
void CodeGenTileLangMUSA::VisitStmt_(const EvaluateNode *op) {
...
void CodeGenTileLangMUSA::VisitExpr_(const RampNode *op, std::ostream &os) {
...
void CodeGenTileLangMUSA::VisitExpr_(const BufferLoadNode *op,
                                     std::ostream &os) { // NOLINT(*)
...
void CodeGenTileLangMUSA::VisitExpr_(const BroadcastNode *op,
                                     std::ostream &os) { // NOLINT(*)
  int lanes = static_cast<int>(Downcast<IntImm>(op->lanes)->value);
...
inline void PrintConst(const FloatImmNode *op, std::ostream &os,
  ...
  case 32: {
    ...
    if (std::isinf(op->value)) {
      ...
      temp << ((op->dtype.bits() == 32) ? "MUSART_INF_F" : "MUSART_INF");
      p->need_math_constants_h_ = true;
    } else if (std::isnan(op->value)) {
      temp << ((op->dtype.bits() == 32) ? "MUSART_NAN_F" : "MUSART_NAN");
      p->need_math_constants_h_ = true;
    } else {
...
void CodeGenTileLangMUSA::VisitExpr_(const FloatImmNode *op,
                                     std::ostream &os) { // NOLINT(*)
  PrintConst(op, os, this);
}

void CodeGenTileLangMUSA::PrintWmmaScope(const std::string &scope, DataType t,
  ...
  if (scope == "wmma.matrix_a") {
    ...
    os << "mtmusa::wmma::fragment<mtmusa::wmma::matrix_a, " << shape_str << ", "
       << type.str() << ", mtmusa::wmma::" << layout_str << ">";
  } else if (scope == "wmma.matrix_b") {
    std::string layout_str = fragment_layouts[variable];
    ICHECK_NE(layout_str, "") << "Layout must be defined for matrix_b";
    os << "mtmusa::wmma::fragment<mtmusa::wmma::matrix_b, " << shape_str << ", "
       << type.str() << ", mtmusa::wmma::" << layout_str << ">";
  } else if (scope == "wmma.accumulator") {
    os << "mtmusa::wmma::fragment<mtmusa::wmma::accumulator, " << shape_str
       << ", " << type.str() << ">";
  }
}

int32_t CodeGenTileLangMUSA::GetWmmaFragmentSize(const std::string &scope,
                                                 const VarNode *variable,
                                                 int32_t size) {
  ICHECK(fragment_shapes.count(variable))
      << "Cannot find shape of the wmma fragment " << variable->name_hint;
...
void CodeGenTileLangMUSA::HandleVolatileLoads(const std::string &value,
...
void CodeGenTileLangMUSA::PrintVecElemLoadExpr(DataType t, int i,
                                               const std::string &value,
                                               std::ostream &os) {
  ICHECK_GT(t.lanes(), 1);
  if (t.bits() == 8 && (t.is_int() || t.is_uint())) {
...
void CodeGenTileLangMUSA::PrintFunctionSignature(const String &function_name,
                                                 const PrimFunc &func,
                                                 std::ostream &os) {
  PrintFuncPrefix(os);
...
void CodeGenTileLangMUSA::AddFunction(const GlobalVar &gvar,
                                      const PrimFunc &f) {
