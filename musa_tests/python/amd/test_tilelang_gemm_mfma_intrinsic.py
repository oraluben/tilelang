...
def assert_tl_matmul_correctness(M,
    ...
    B_shape = (N, K) if b_transposed else (K, N)
    if in_dtype == "int8":
        A = torch.randint(-128, 127, A_shape, device="musa", dtype=torch.int8)
        B = torch.randint(-128, 127, B_shape, device="musa", dtype=torch.int8)
    else:
        A = torch.rand(A_shape, device="musa", dtype=getattr(torch, in_dtype))
        B = torch.rand(B_shape, device="musa", dtype=getattr(torch, in_dtype))
