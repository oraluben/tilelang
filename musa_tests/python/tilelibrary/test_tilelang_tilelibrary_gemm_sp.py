...
def run_gemm_sp(
    ...
    A = randn_semi_sparse(M, K, dtype=STR_TO_TYPE[in_dtype], device="musa", transposed=trans_A)
    if trans_B:
        B = torch.randn((N, K), device="musa", dtype=torch.float32)
    else:
        B = torch.randn((K, N), device="musa", dtype=torch.float32)
