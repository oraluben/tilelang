...
def sparse_mla_fwd(
    ...
    def main(
        ...
        with T.Kernel(seq_len * REPLICATE_H, batch, kv_group, threads=threads) as (
            ...
            KV_shared_for_transpose = T.alloc_shared([BI, D], dtype)
            KV_shared_for_no_transpose = T.alloc_shared([BI, D], dtype)
            ...
            for i_i in T.Pipelined(NI, num_stages=num_stages):
                ...
                for bi_i, d_i in T.Parallel(BI, D):
                    kv_val = KV[b_i, Indices[b_i, s_i, g_i, i_i * BI + bi_i], g_i, d_i]
                    KV_shared_for_transpose[bi_i, d_i] = kv_val
                    KV_shared_for_no_transpose[bi_i, d_i] = kv_val
                ...
                T.gemm(S_shared, KV_shared_for_no_transpose, acc_o)
...
def ref_sparse_mla_fwd_interface(q, kv, indices, sm_scale=None, is_casual=True):
    ...
    g_index = g
    h_index = h // g
    compressed_casual_mask = torch.arange(0, sq, dtype=torch.int32, device="musa").view(
        -1, 1
    ) >= torch.arange(1 - 1, sk * 1, 1, dtype=torch.int32, device="musa").view(1, -1)
...
def test_sparse_mla_fwd(
    ...
    kv = torch.randn((B, SKV, HKV, DQK), dtype=dtype, device="musa").requires_grad_(
        True
    )

    indices = torch.full((B, S, HKV, topk), SKV, dtype=torch.int32, device="musa")
    for b in range(B):
        for t in range(S):
            for h in range(HKV):
